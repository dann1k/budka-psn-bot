...
- `supabase/functions/telegram-webhook/assets/` — исходные renderer assets и лицензии; при обновлении шрифтов или wasm из них нужно регенерировать `renderer-assets.ts` через `npm run renderer:assets`.
...
- В `/popular` участники выводятся без `@`, чтобы не отправлять им Telegram-уведомления; строки топа оформлены Telegram-цитатами, как в `/table`. `/popular debug uncharted` показывает найденные игровые бакеты по строке поиска, PSN-аккаунты, из которых они пришли, resolved `accountId`, число загруженных trophy titles и проверенные аккаунты без совпадений.
