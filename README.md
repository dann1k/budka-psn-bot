...
- `/popular debug` — тот же топ, плюс причины пропуска недоступных PSN-аккаунтов.
...
- `/popular` сканирует played games всех привязанных аккаунтов за всё время, склеивает версии игры через PSN `concept.id` и считает одного Telegram-участника один раз, даже если игра есть на нескольких его PSN-аккаунтах. Рейтинг сортируется по числу участников, затем по названию игры.
- В `/popular` участники выводятся без `@`, чтобы не отправлять им Telegram-уведомления; `/popular debug` дополнительно показывает до 10 пропущенных аккаунтов и краткую причину.
